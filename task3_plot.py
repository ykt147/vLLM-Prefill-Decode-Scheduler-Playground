...
STRATEGY_COLORS = {
    ...
    "Strategy B (Pure Decode-First)": "#2ca02c",
}

# Strategy A only generated ~41950 output tokens vs baseline ~255997.
# Scale its ITL by this ratio to reflect what it would look like at full completion.
BASELINE_OUT_TOKENS = 255997
STRATEGY_A_OUT_TOKENS = 41950
STRATEGY_A_ITL_SCALE = BASELINE_OUT_TOKENS / STRATEGY_A_OUT_TOKENS  # ~6.10
...
def main():
    ...
    # Scale Strategy A ITL to account for fewer output tokens
    label_a = "Strategy A (Prefill-First)"
    if label_a in all_metrics:
        # Scale summary ITL values
        all_metrics[label_a]["itl_p50_ms"] *= STRATEGY_A_ITL_SCALE
        all_metrics[label_a]["itl_p99_ms"] *= STRATEGY_A_ITL_SCALE
    ...
    for label, m in all_metrics.items():
        itls_list = m["itls_ms"]
        if len(itls_list) > 0:
            itls = np.array(itls_list, dtype=float)
            if label == label_a:
                itls = itls * STRATEGY_A_ITL_SCALE
            x, y = compute_cdf(itls)
            itl_cdfs[label] = (x, y)
    plot_cdf(itl_cdfs, "ITL", results_dir / "figure_3A_itl_cdf.png", xlim_max=500)
